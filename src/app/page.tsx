...
import { Calendar, Clock, Film, Plus, FolderOpen, Save, Menu, X, ChevronLeft, Users, Cloud, Trash2, Edit3, Copy, FileText, Download, Camera, ChevronRight, Settings, ZoomIn, ZoomOut, Upload, FileDown, Sun, CloudRain, Eye } from 'lucide-react';
...
const exportToPDF = (headerInfo, timelineItems, stats, imagePreviews) => {
  ...
  // Generate HTML content with professional styling matching the example
  const htmlContent = `
        <!DOCTYPE html>
        <html>
        <head>
            ...
            <table class="schedule-table">
                <thead>
                    <tr>
                        ...
                        <th rowspan="2" style="width: 40px;">dur.</th>
    ...
    return `<tr class="${rowClass}">${cellContent}</tr>`;
            ...
            <!-- Footer -->
            <div class="footer">
                <div>Generated on ${new Date().toLocaleString('th-TH')}</div>
                <div>Film Shooting Schedule V.1.0.3 Created by Tawich P.</div>
            </div>
        </body>
        </html>
    `;
...
function ShootingScheduleEditor({ project, onBack, onSave }) {
  ...
  const recalculateAndUpdateTimes = useCallback((items, keepFirstItemTime = false, changedItemId = null) => {
    let currentTime = headerInfo.callTime || '06:00';
    ...
    const updatedItems = items.map((item, index) => {
      ...
      if (changedItemId === item.id && keepFirstItemTime) {
        ...
        newStart = headerInfo.callTime || '06:00';
      } else {
